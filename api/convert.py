...
Incluye los 4 extractores:
  1) extract_original         (facturas & proformas clásicas)
  2) extract_slice            (layout por columnas fijas)
  3) extract_new_provider     (proveedor Dior “No. Description … Each”)
  4) extract_tepf_scalp       (líneas TE/PF … UN xx … gencod)
"""
...
def extract_original(pdf_path: str) -> List[dict]:
    ...
    # Propaga país de origen si está en solo una fila por factura
        ...
        for page in pdf.pages:
            ...
            for i, line in enumerate(lines):
                if m := TEPF_REGEX.match(line):
                    ...
                    # gencod en las 2 líneas siguientes
                    gencod = ""
                    ...
                    rows.append({
                        ...
                        "Invoice Number": "",  # se rellenará con inv_num si lo tenemos
                        "gencod": gencod
                    })
    return rows
...
@app.post("/")          # segundo decorador en línea aparte
def convert():
    try:
        ...
        for pdf in files_in:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
                ...
                # intenta sacar número de factura del nombre (SIP123456)
                ...
                with pdfplumber.open(tmp.name) as pdf_for_name:
                    txt_all = "\n".join(p.extract_text() or "" for p in pdf_for_name.pages)
                ...
                for r in o4:  # añade el número de invoice si lo sacamos del filename
