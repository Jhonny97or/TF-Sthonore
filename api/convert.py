...
# ─── 2) REGEX Y HELPERS ───────────────────────────────────────────
INV_PAT  = re.compile(
    r'(?:FACTURE|INVOICE)[^\d]{0,300}?(\d{6,})',  # ← ampliado a 300 chars
    re.I | re.S                                   #   y se permiten saltos de línea
)
...
def convert():
    try:
        ...
        for pdf_file in pdfs:
            ...
            with pdfplumber.open(tmp.name) as pdf:
                for page in pdf.pages:
                    ...
                    # 3.1  ¿Número de factura en esta página?
                    ...
                    # 3.2  País de origen (línea o línea siguiente)
        ...
        # 4)  Completar Origin vacío si es único por factura
